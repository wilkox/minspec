...
until (keys(%assemblage) == $sizeOfAssemblage) {
	...
	my $abundance = 1 / (log(keys(%assemblage) + 2));
...
my %falseTaxa; #count how many false taxa are introduced
...
foreach (@minspecOutput) {
  ...
  #to count the number of taxa minspec correctly
  #identified as false
  if ($status == 0) {
    ++$removedCorrectly unless exists $assemblage{$taxon};
  }
...
if ($falseTaxa == 0) {
  ...
  $falsePositiveRate = abs((100 * $falsePositive) / ($falseTaxa));
}
